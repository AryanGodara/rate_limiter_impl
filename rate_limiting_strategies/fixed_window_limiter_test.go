...
func TestFixedWindowLimiter_Execute(t *testing.T) {
	tt := []struct {
		desc        string
		runs        int64
		...
		{
			desc: "denies requests exceeding the limit",
			req: &rate_limiter_impl.Request{
				Key:      "test",
				Limit:    100,
				Duration: time.Minute,
		...
		{
			desc: "resets and starts anew after TTL expiry",
			req: &rate_limiter_impl.Request{
				Key:      "test",
				Limit:    100,
				Duration: time.Minute,
			...
			res: &rate_limiter_impl.Result{
				State:         rate_limiter_impl.Allow,
				TotalRequests: 39,
				ExpiresAt:     time.Date(2024, time.June, 23, 10, 17, 32, 0, time.Local),
			},
			runs:        100,
	...
	for _, ts := range tt {
		...
		counter := NewFixedWindowLimiter(client, func() time.Time {
			return now
		})
